...
export function WorkDetail({ slug }: { slug: string }) {
    ...
    return (
      <div className="page-shell py-20">
        <div className="glass-panel rounded-[28px] p-8 text-center">
          ...
          <Link
            href="/create"
            className="gold-button mt-6 inline-block px-5 py-3"
          >
            重新创作
          </Link>
        </div>
  ...
  if (!artwork) {
    return (
      <div className="page-shell py-20 text-lg font-black">
        正在加载作品提案...
      </div>
    );
  }
  ...
  const shareText = `我用「非遗有灵」生成了一套${artwork.craftName || "非遗"}文创提案：${artwork.prompt || ""}`;
  ...
  async function shareWork() {
    ...
    // Priority 2: Copy link to clipboard
    try {
    ...
    <main className="page-shell py-10">
      <section className="mb-6 flex flex-col justify-between gap-5 lg:flex-row lg:items-end">
        ...
        <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:items-center sm:gap-3">
          <div className="flex items-center gap-1 rounded-full bg-white/8 px-3 py-2 text-sm font-bold text-[var(--foreground-dim)]">
          ...
          <button
            onClick={shareWork}
            className="quiet-button flex items-center justify-center gap-2 px-4 py-3"
            aria-label="分享作品链接"
          >
            {copied ? <Check size={17} className="text-[var(--cyan)]" /> : <Share2 size={17} />}
            {copied ? "已复制" : "分享作品"}
          </button>
          {/* 下载提案 — 导出提案卡片为图片 */}
          <button
          ...
          <button
            ...
            className="gold-button col-span-2 flex items-center justify-center gap-2 px-4 py-3 sm:col-span-1"
          >
            <Download size={17} />
            导出海报
          </button>
      ...
      <section
        id="proposal-card"
        className="glass-panel grid gap-6 rounded-[34px] p-5 lg:grid-cols-[0.9fr_1.1fr]"
      >
        <div className="rounded-[28px] bg-[var(--bg-deep)] p-4">
          {patternImage ? (
            ...
            <div className="grid aspect-square place-items-center rounded-[22px] bg-[var(--bg-elevated)] text-3xl font-black text-[var(--gold-soft)]">
              非遗有灵
            </div>
          )}
        </div>
        <div className="flex flex-col gap-4">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {[
              [
                "文化符号",
                artwork.narrative.symbol,
                artwork.narrative.meaning,
              ],
              ["应用场景", "文旅礼品", artwork.narrative.scenario],
              [
                "落地价值",
                "可分享可再创作",
                artwork.narrative.value,
              ],
          ...
          <div className="grid gap-3 sm:grid-cols-4">
            {products.map((product) => (
              <div
                key={product}
                className="rounded-2xl bg-[var(--bg-elevated)] p-3 text-center text-sm font-black"
              >
                {product}
              </div>
            ))}
      ...
      <section className="mt-8 grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
        ...
        <aside className="glass-panel rounded-[28px] p-5">
          ...
          <div className="mt-5 inline-block rounded-2xl bg-white p-3">
            <QRCodeSVG
              value={shareUrl || "https://lively-heritage.local"}
              size={132}
            />
          </div>
      ...
      <section className="mt-8">
        ...
        <div className="grid gap-4 lg:grid-cols-[1fr_1fr]">
          ...
          <div className="flex flex-col gap-4">
            <div className="glass-panel rounded-[28px] p-6">
              <div className="text-xs font-black uppercase tracking-wide text-[var(--gold)]">
                纹样寓意 · {artwork.narrative.symbol}
              </div>
              <p className="mt-3 text-sm leading-7 text-[var(--foreground-dim)]">
                {craftInfo.motifs[0].story}
              </p>
            </div>
            <div className="glass-panel rounded-[28px] p-6">
              <div className="text-xs font-black uppercase tracking-wide text-[var(--gold)]">
