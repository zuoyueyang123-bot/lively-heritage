...
export function AppHeader() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-[var(--line)] bg-[#fffaf2]/82 backdrop-blur-xl">
      <div className="page-shell flex h-16 items-center justify-between gap-5">
        <Link href="/" className="flex items-baseline gap-3" onClick={() => setOpen(false)}>
          ...
          <span className="hidden text-xs font-bold uppercase tracking-[0.24em] text-[var(--muted)] sm:inline">
            Lively Heritage
          </span>
        </Link>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => setOpen((v) => !v)}
            className="grid h-10 w-10 place-items-center rounded-full border border-[var(--line)] bg-white/70 md:hidden"
      ...
      {/* Mobile nav drawer */}
      {open && (
        <nav className="border-t border-[var(--line)] bg-[#fffaf2]/95 backdrop-blur-xl md:hidden">
          <div className="page-shell flex flex-col py-3">
            {links.map((link) => (
              <Link
                ...
                className="rounded-xl px-4 py-3 text-base font-bold text-[var(--ink)] transition hover:bg-black/5"
              >
                {link.label}
              </Link>
            ))}
